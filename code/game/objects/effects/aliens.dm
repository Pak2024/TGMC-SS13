...
/mob/living/carbon/human/proc/acid_spray_entered(datum/source, obj/effect/xenomorph/spray/acid_spray, acid_damage, slow_amt)
	...
	if(acid_spray.xeno_owner && TIMER_COOLDOWN_FINISHED(acid_spray, COOLDOWN_PARALYSE_ACID)) //To prevent being able to walk "over" acid sprays
		acid_spray_act(acid_spray.xeno_owner)
		return

	if(TIMER_COOLDOWN_FINISHED(src, COOLDOWN_ACID))
		return
